...
import { CHECKPOINTS, RACE_START, RACE_DIFFICULTIES, buildAiLine, RITA_RACE_LINES, type RaceDifficulty, type RacePoint } from '../data/race';
...
class RaceSystem {
  ...
  start(diff: RaceDifficulty): void {
    ...
    // grid up: player buggy on the inside line, Rita on the outside
    this.hooks.forceDrive();
    vehicles.buggy.place(RACE_START.player.x, RACE_START.player.z, RACE_START.yaw);
    ...
    this.rival.place(RACE_START.rival.x, RACE_START.rival.z, RACE_START.yaw);
    ...
    this.aiLine = buildAiLine(cut1, cut2);

    this.buildGates();
    this.setHud(true);
    this.hooks.banner('REDLINE’S RUN');
  ...
  update(dt: number): void {
    ...
    if (this.rival) {
      ...
      // rival checkpoint progress
      if (this.aiCp < CHECKPOINTS.length) {
        const cp = CHECKPOINTS[this.aiCp];
        if (Math.hypot(this.rival.pos.x - cp.x, this.rival.pos.z - cp.z) < cp.r + 6) this.aiCp++;
        if (this.aiCp >= CHECKPOINTS.length && !this.aiFinished) {
    ...
    if (vehicles.buggy && this.playerCp < CHECKPOINTS.length) {
      const cp = CHECKPOINTS[this.playerCp];
      const p = this.hooks.playerPos();
      if (Math.hypot(p.x - cp.x, p.z - cp.z) < cp.r) {
        this.passGate(this.playerCp);
        this.playerCp++;
        if (this.playerCp >= CHECKPOINTS.length) { this.finish(); return; }
      }
    }
  ...
  private playerProgress(): number {
    ...
    const p = this.hooks.playerPos();
    const cp = CHECKPOINTS[Math.min(this.playerCp, CHECKPOINTS.length - 1)];
    return this.playerCp * 1000 - Math.hypot(p.x - cp.x, p.z - cp.z);
  }
  private aiProgress(): number {
    if (!this.rival) return 0;
    const cp = CHECKPOINTS[Math.min(this.aiCp, CHECKPOINTS.length - 1)];
    return this.aiCp * 1000 - Math.hypot(this.rival.pos.x - cp.x, this.rival.pos.z - cp.z);
  }
  ...
  private passGate(i: number): void {
    ...
    const g = this.gates[i];
    if (g) {
      fx.burst(g.ring.position.clone(), 0x3ddc4e, 24, 8, 0.14, 0.8, 2);
      this.hooks!.scene.remove(g.group);
    }
    const left = CHECKPOINTS.length - 1 - i;
    if (left > 0) this.hooks!.toast(`CHECKPOINT — <b>${left}</b> to go`, '#54d4ff');
  }

  // ------------------------------------------------------------------ finish
  private finish(): void {
    if (!this.hooks) return;
    const won = !this.aiFinished; // Rita already home = you lost
    const diff = this.difficulty;
    const store = loadStore();
    const prevBest = store.best[diff.id];
    const isBest = prevBest === undefined || this.raceMs < prevBest;
    if (isBest) store.best[diff.id] = Math.round(this.raceMs);
    ...
    if (won) {
      ...
      this.hooks.toast(`REDLINE’S RUN (${diff.name}) — <b>$${diff.rewardCash}</b> · <b>${diff.rewardXp} XP</b> · ${formatRaceTime(this.raceMs)}${isBest ? ' · <b style="color:#ffd23c">TRACK RECORD</b>' : ''}`, '#3ddc4e');
      if (diff.firstWinItem && !store.wins[diff.id]) {
        this.hooks.grantItem(diff.firstWinItem);
      }
      store.wins[diff.id] = true;
      ...
      this.hooks.toast(`Rita by ${formatRaceTime(this.raceMs - this.aiFinishMs)} — consolation <b>$${consolation}</b>. Rematch anytime.`, '#ff8c5a');
  ...
  private updateHud(): void {
    ...
    this.hud.innerHTML = `
      <div style="font-size:24px; font-weight:800; color:${place === 1 ? '#3ddc4e' : '#ff8c5a'}">${place === 1 ? '1st' : '2nd'} <span style="opacity:0.6; font-size:15px;">of 2</span></div>
      <div style="font-size:14px; margin-top:2px;">GATE ${Math.min(this.playerCp + 1, CHECKPOINTS.length)}/${CHECKPOINTS.length} · ${formatRaceTime(this.raceMs)}</div>`;
  }
}
