...
import { ENEMIES, BOSS_GUTTERBALL, BOSS_WARDEN, type EnemyDef } from '../data/enemies';
...
// ---------------------------------------------------------------------------
export function spawnBoss(id: 'gutterball' | 'warden_prime', pos: THREE.Vector3): Enemy {
  const level = state.level + 2;
  const boss = id === 'gutterball' ? new Gutterball(level, pos) : new WardenPrime(level, pos);
  enemySpawner.registerBoss(boss);
  fx.explosion(pos.clone().add(new THREE.Vector3(0, 1, 0)), 4, id === 'gutterball' ? 0xff8438 : 0x54d4ff);
  return boss;
}
